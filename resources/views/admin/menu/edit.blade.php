...
@section('content')
    <div class="p-8">
        <h2 class="text-4xl mb-6 font-extrabold dark:text-white">Edit Menu</h2>
        {{-- header image --}}
        <form method="post" action="{{ route('menu.update', $item->id) }}" enctype="multipart/form-data">
            ...
            <div class="grid p-5 mb-6 rounded-lg gap-4 grid-cols-2 bg-white">
                ...
                <div class="flex flex-col">
                    ...
                    <div class="mb-6">
                        <div class="relative">
                            <input type="text" id="floating_outlined" value="{{ $item->name }}" name="name"
                    ...
                    <div class="mb-6">
                        <div class="flex">
                            ...
                            <div class="relative">
                                <input type="number" id="floating_outlined" value="{{ $item->price }}" name="price"
                        ...
                        <select id="category" name="categories_id"
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}"
                                        @if($item->categories->categories_id == ($category->id)) selected @endif >
                                    {{ $category->name }}
                                </option>
                            @endforeach 
                        </select>
